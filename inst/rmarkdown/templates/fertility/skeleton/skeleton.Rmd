...
In this lab we will calculate various birth rates in the US from 1933 to 2017.  The very first step is to load the necessary libraries in R.  What this does gives R information on what each command means, sort of like loading a dictionary that R can check commands against so it knows what to do.
...
To calculate the birth rate, first we need to get the total population by year.  Mortality.org provides demographic data on a number of countries.  You'll need a username and password for mortality.org to access the data.  If you have not done so already, please visit mortality.org and create an account.  On their homepage on the left hand side, you'll see an option to create a "New User".  In the code below, we will pull and format the yearly population from 1933 to 2017 in the USA.  You only need to replace the placeholders "YOUR MORTALITY.ORG USERNAME" and "YOUR MORTALITY.ORG PASSWORD" with your username and password for the site.  No other changes are necessary in this chunk.  Once you've entered in your username and password, highlight and run the lines below.
...
Let's take a look at what this data looks like.  Entering the name of the dataset will display the data saved in this dataset in your console.  As you'll see, we now have the population, births, births/population, and birth rate for each year from 1933 to 2017.
...
ggplot(birthdata, aes(x = Year, y = birth_rate)) + geom_line() + ggtitle("US Birth Rate 1933-2017")
...
agespecificbirths <- readHFDweb(CNTRY='USA', item='birthsRR', username='YOUR HUMANFERTILITY.ORG USERNAME', password='YOUR HUMANFERTILITY.ORG PASSWORD')
...
ASFRdata <- subset(ASFRdata, Year<=2017)
...
(HINT: We are interested only in the female population, not the total population in each year.)
...
Let's take a look at what ASFR has looked like across the years 1933 to 2017.

```{r}

ggplot(data = ASFRdata, aes(x = Age, y = ASFR, color=Year)) + geom_line(aes(group=Year)) + ggtitle("US ASFR 1933-2017")
...
Now let's graph the TFR from 1933-2017.

```{r}

ggplot(TFRdata, aes(x = Year, y = TFR)) + geom_line() + ggtitle("US Total Fertility Rate 1933-2017")

```

Once you've completed all chunks in this lab, you are ready to knit this template to PDF.
