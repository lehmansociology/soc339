...
Similar to what we did in the internal migration lab, we'll start by creating geographies.  What this does is tell R which states we want data for.  Then we pull the data from the ACS.  Enter in the year 2017 in lines 31 and 32 where prompted to tell the ACS we want data for the year 2017.
...
race2017 <- acs.fetch(geography = selectedstates, table.number = "B02001", 
...
race2017.names <- acs.colnames(race2017)
new.names <- stringr::str_sub(race2017.names, start = 7)
acs.colnames(race2017) <- new.names

race <- as.data.frame(estimate(race2017))
